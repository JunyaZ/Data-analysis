...
def Getinfo(filename,snp,infile):
    ...
    for df in pd.read_csv(infile,header=None, delimiter=' ',chunksize=10000000):
        ...
        df = df[df[6] != '-']
        df = df[df[5] != '-']
        df = df[df[0].isin(list_)]
        df = df[df[2].isin(snplist)]
        df.to_csv(Output + "gwas_1mv1.csv",mode='a',index= False,header=None)
        p1=df.groupby(0)
        print(p1.ngroups)
...
#    df =Getinfo(filename,snp,infile)
    ...
    thenewlist = [x for x in allFiles if x.split(sep="\\")[-1:][0] not in check]
